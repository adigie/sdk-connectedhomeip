...
CHIP_ERROR ServerImpl::Init(const ServerInitParams & initParams)
{
    ...
    SuccessOrExit(err = mCommissioningWindowManager.Init(&mParent));
    ...
    // everything is initialized, so we can do a deferred clean-up.
    {
        ...
        if (fabricIndexDeletedOnInit != kUndefinedFabricIndex)
        {
            ...
            PlatformMgr().ScheduleWork(
                [](intptr_t arg) {
                    ServerImpl * server = reinterpret_cast<ServerImpl *>(arg);
...
void ServerImpl::OnPlatformEvent(const DeviceLayer::ChipDeviceEvent & event)
{
    switch (event.Type)
    {
...
void ServerImpl::CheckServerReadyEvent()
{
...
void ServerImpl::OnPlatformEventWrapper(const DeviceLayer::ChipDeviceEvent * event, intptr_t server)
{
    reinterpret_cast<ServerImpl *>(server)->OnPlatformEvent(*event);
}

void ServerImpl::RejoinExistingMulticastGroups()
{
...
#if CHIP_CONFIG_ENABLE_ICD_CIP
bool ServerImpl::ShouldCheckInMsgsBeSentAtBootFunction(FabricIndex aFabricIndex, NodeId subjectID)
{
#if CHIP_CONFIG_PERSIST_SUBSCRIPTIONS
...
void ServerImpl::GenerateShutDownEvent()
{
    PlatformMgr().ScheduleWork([](intptr_t) { PlatformMgr().HandleServerShuttingDown(); });
}

void ServerImpl::ScheduleFactoryReset()
{
...
CHIP_ERROR ServerImpl::SendUserDirectedCommissioningRequest(chip::Transport::PeerAddress commissioner,
...
#if CHIP_CONFIG_PERSIST_SUBSCRIPTIONS
void ServerImpl::ResumeSubscriptions()
{
...
Credentials::IgnoreCertificateValidityPeriodPolicy ServerImpl::sDefaultCertValidityPolicy;
