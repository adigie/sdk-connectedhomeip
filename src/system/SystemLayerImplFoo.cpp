...
#include <system/SystemLayerImplFoo.h>
...
CHIP_ERROR LayerImplFoo::StartTimer(Clock::Timeout delay, TimerCompleteCallback onComplete, void * appState)
{
    ...
    //         dispatch_source_t timerSource = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, DISPATCH_TIMER_STRICT,
    //         dispatchQueue); VerifyOrDie(timerSource != nullptr);
    //
    //         timer->mTimerSource = timerSource;
    //         dispatch_source_set_timer(
    //             timerSource, dispatch_walltime(nullptr, static_cast<int64_t>(Clock::Milliseconds64(delay).count() *
    //             NSEC_PER_MSEC)), DISPATCH_TIME_FOREVER, 2 * NSEC_PER_MSEC);
    ...
    //     ev_timer_init(&timer->mLibEvTimer, &LayerImplFoo::HandleLibEvTimer, 1, 0);
...
CHIP_ERROR LayerImplFoo::ExtendTimerTo(Clock::Timeout delay, TimerCompleteCallback onComplete, void * appState)
{
...
bool LayerImplFoo::IsTimerActive(TimerCompleteCallback onComplete, void * appState)
{
...
Clock::Timeout LayerImplFoo::GetRemainingTime(TimerCompleteCallback onComplete, void * appState)
{
    // return mTimerList.GetRemainingTime(onComplete, appState);
    return {};
}

void LayerImplFoo::CancelTimer(TimerCompleteCallback onComplete, void * appState)
{
    //     assertChipStackLockedByCurrentThread();
    //
...
CHIP_ERROR LayerImplFoo::ScheduleWork(TimerCompleteCallback onComplete, void * appState)
{
    //     assertChipStackLockedByCurrentThread();
    //
    //     VerifyOrReturnError(mLayerState.IsInitialized(), CHIP_ERROR_INCORRECT_STATE);
...
// CHIP_ERROR LayerImplFoo::StartWatchingSocket(int fd, SocketWatchToken * tokenOut)
// {
//     // Find a free slot.
...
//     watch->mFD = fd;
// #if CHIP_SYSTEM_CONFIG_USE_LIBEV
//     ev_io_init(&watch->mIoWatcher, &LayerImplFoo::HandleLibEvIoWatcher, 0, 0);
//     watch->mIoWatcher.data = watch;
//     watch->mLayerImplFooP  = this;
// #endif
//
...
// CHIP_ERROR LayerImplFoo::SetCallback(SocketWatchToken token, SocketWatchCallback callback, intptr_t data)
// {
...
// CHIP_ERROR LayerImplFoo::RequestCallbackOnPendingRead(SocketWatchToken token)
// {
//     SocketWatch * watch = reinterpret_cast<SocketWatch *>(token);
//     VerifyOrReturnError(watch != nullptr, CHIP_ERROR_INVALID_ARGUMENT);
//
...
// CHIP_ERROR LayerImplFoo::RequestCallbackOnPendingWrite(SocketWatchToken token)
// {
//     SocketWatch * watch = reinterpret_cast<SocketWatch *>(token);
//     VerifyOrReturnError(watch != nullptr, CHIP_ERROR_INVALID_ARGUMENT);
//
...
// CHIP_ERROR LayerImplFoo::ClearCallbackOnPendingRead(SocketWatchToken token)
// {
//     SocketWatch * watch = reinterpret_cast<SocketWatch *>(token);
//     VerifyOrReturnError(watch != nullptr, CHIP_ERROR_INVALID_ARGUMENT);
//
...
// CHIP_ERROR LayerImplFoo::ClearCallbackOnPendingWrite(SocketWatchToken token)
// {
//     SocketWatch * watch = reinterpret_cast<SocketWatch *>(token);
//     VerifyOrReturnError(watch != nullptr, CHIP_ERROR_INVALID_ARGUMENT);
//
...
// CHIP_ERROR LayerImplFoo::StopWatchingSocket(SocketWatchToken * tokenInOut)
// {
 ...
 */
// SocketEvents LayerImplFoo::SocketEventsFromFDs(int socket, const fd_set & readfds, const fd_set & writefds,
...
// void LayerImplFoo::AddLoopHandler(EventLoopHandler & handler)
// {
...
// void LayerImplFoo::RemoveLoopHandler(EventLoopHandler & handler)
// {
...
// void LayerImplFoo::PrepareEvents()
// {
//     assertChipStackLockedByCurrentThread();
//
...
void LayerImplFoo::HandleTimerComplete(TimerList::Node * timer)
{
...
void LayerImplFoo::HandleLibEvTimer(EV_P_ struct ev_timer * t, int revents)
{
    TimerList::Node * timer = static_cast<TimerList::Node *>(t->data);
    VerifyOrDie(timer != nullptr);
    LayerImplFoo * layerP = dynamic_cast<LayerImplFoo *>(timer->mCallback.mSystemLayer);
    VerifyOrDie(layerP != nullptr);
    layerP->mTimerList.Remove(timer);
    layerP->mTimerPool.Invoke(timer);
}

void LayerImplFoo::HandleLibEvIoWatcher(EV_P_ struct ev_io * i, int revents)
{
    SocketWatch * watch = static_cast<SocketWatch *>(i->data);
    if (watch != nullptr && watch->mCallback != nullptr && watch->mLayerImplFooP != nullptr)
    {
        SocketEvents events;
        if (revents & EV_READ)
        {
...
// void LayerImplFoo::SocketWatch::Clear()
// {
//     mFD = kInvalidFd;
//     mPendingIO.ClearAll();
//     mCallback     = nullptr;
...
//     mWrSource = nullptr;
// #elif CHIP_SYSTEM_CONFIG_USE_LIBEV
//     mLayerImplFooP = nullptr;
// #endif
// }

#if CHIP_SYSTEM_CONFIG_USE_DISPATCH
void LayerImplFoo::SocketWatch::DisableAndClear()
{
    if (mRdSource)
    {
...
void LayerImplFoo::SocketWatch::DisableAndClear()
{
    if (mLayerImplFooP != nullptr && mLayerImplFooP->mLibEvLoopP != nullptr)
    {
        ev_io_stop(mLayerImplFooP->mLibEvLoopP, &mIoWatcher);
    }
    Clear();
}
#endif // CHIP_SYSTEM_CONFIG_USE_DISPATCH/LIBEV
