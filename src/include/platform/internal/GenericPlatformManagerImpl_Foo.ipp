 ...
 *          GenericPlatformManagerImpl_Foo<> template.
 */

#ifndef GENERIC_PLATFORM_MANAGER_IMPL_FOO_CPP
#define GENERIC_PLATFORM_MANAGER_IMPL_FOO_CPP

#include <platform/PlatformManager.h>
#include <platform/internal/CHIPDeviceLayerInternal.h>
#include <platform/internal/GenericPlatformManagerImpl_Foo.h>

// Include the non-inline definitions for the GenericPlatformManagerImpl<> template,
// from which the GenericPlatformManagerImpl_Foo<> template inherits.
...
K_WORK_DEFINE(sSignalWork, [](k_work *) {});

} // anonymous namespace

template <class ImplClass>
CHIP_ERROR GenericPlatformManagerImpl_Foo<ImplClass>::_InitChipStack(void)
{
...
template <class ImplClass>
void GenericPlatformManagerImpl_Foo<ImplClass>::_LockChipStack(void)
{
    k_mutex_lock(&mChipStackLock, K_FOREVER);
}

template <class ImplClass>
bool GenericPlatformManagerImpl_Foo<ImplClass>::_TryLockChipStack(void)
{
    return k_mutex_lock(&mChipStackLock, K_NO_WAIT) == 0;
}

template <class ImplClass>
void GenericPlatformManagerImpl_Foo<ImplClass>::_UnlockChipStack(void)
{
    k_mutex_unlock(&mChipStackLock);
}

template <class ImplClass>
CHIP_ERROR GenericPlatformManagerImpl_Foo<ImplClass>::_StartChipTimer(System::Clock::Timeout delay)
{
...
template <class ImplClass>
CHIP_ERROR GenericPlatformManagerImpl_Foo<ImplClass>::_StopEventLoopTask(void)
{
    mShouldRunEventLoop = false;
    return CHIP_NO_ERROR;
}
...
template <class ImplClass>
void GenericPlatformManagerImpl_Foo<ImplClass>::_Shutdown(void)
{
#ifdef CONFIG_REBOOT
...
template <class ImplClass>
CHIP_ERROR GenericPlatformManagerImpl_Foo<ImplClass>::_PostEvent(const ChipDeviceEvent * event)
{
...
template <class ImplClass>
void GenericPlatformManagerImpl_Foo<ImplClass>::ProcessDeviceEvents()
{
...
template <class ImplClass>
void GenericPlatformManagerImpl_Foo<ImplClass>::_RunEventLoop(void)
{
    Impl()->LockChipStack();
...
template <class ImplClass>
void GenericPlatformManagerImpl_Foo<ImplClass>::EventLoopTaskMain(void * thisPtr, void *, void *)
{
    ChipLogProgress(DeviceLayer, "CHIP task running");
    static_cast<GenericPlatformManagerImpl_Foo<ImplClass> *>(thisPtr)->Impl()->RunEventLoop();
}

template <class ImplClass>
CHIP_ERROR GenericPlatformManagerImpl_Foo<ImplClass>::_StartEventLoopTask(void)
{
    if (!mChipThreadStack)
...
template class GenericPlatformManagerImpl_Foo<PlatformManagerImpl>;

} // namespace Internal
} // namespace DeviceLayer
} // namespace chip

#endif // GENERIC_PLATFORM_MANAGER_IMPL_FOO_CPP
